...
-----END PRIVATE KEY-----`;
  ...
  __name(pemToArrayBuffer2, "pemToArrayBuffer");
...
__name(handleGA4Report, "handleGA4Report");
...
<div class="card"><div class="label">\u5E73\u5747\u4F1A\u8BDD\u65F6\u957F</div><div class="value">${fmtDur(aD)}</div></div>
  ...
  if (daily.length >= 2) {
    ...
    const trend = sH > fH ? "\u{1F4C8} \u4E0A\u5347" : sH < fH ? "\u{1F4C9} \u4E0B\u964D" : "\u27A1\uFE0F \u6301\u5E73";
  ...
  if (topPages.length > 0)
    h += `<li><strong>\u6700\u53D7\u6B22\u8FCE\u9875\u9762</strong>: "${topPages[0].title.slice(0, 50)}"\uFF08${topPages[0].views} \u6B21\u6D4F\u89C8\uFF09</li>`;
  if (sources.length > 0)
    h += `<li><strong>\u6700\u5927\u6D41\u91CF\u6E20\u9053</strong>: ${chNames[sources[0].channel] || sources[0].channel}\uFF08${sources[0].sessions} \u4E2A\u4F1A\u8BDD\uFF09</li>`;
  ...
  if (aB > 70)
    h += `<li>\u26A0\uFE0F \u8DF3\u51FA\u7387\u504F\u9AD8 (${aB.toFixed(1)}%) \u2014 \u5EFA\u8BAE\u4F18\u5316\u843D\u5730\u9875</li>`;
  else if (aB > 0 && aB < 40)
    h += `<li>\u2705 \u8DF3\u51FA\u7387\u5065\u5EB7 (${aB.toFixed(1)}%)</li>`;
  h += `</ul></div>`;
...
__name(handleGA4ReportHTML, "handleGA4ReportHTML");
async function handleGA4Debug(env) {
  ...
  let saKey;
  try {
    saKey = JSON.parse(GA4_SA_KEY);
  } catch (e) {
    return jsonResponse({ ...debug, error: "Invalid JSON key", parseError: e.message });
  }
  async function base64url(data) {
    const encoded = btoa(typeof data === "string" ? data : JSON.stringify(data));
    return encoded.replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
...
__name(handleGA4Debug, "handleGA4Debug");
export {
