...
async function handlePublish(request, env, ctx) {
  try {
    ...
    for (const [accountKey, accountConfig2] of Object.entries(BUFFER_ACCOUNTS)) {
      const accountQuota = await checkAccountQuota(env, accountKey);
      if (!accountQuota.allowed) {
        allResults.failed.push(...Object.keys(accountConfig2.channels));
        allResults.details.push({
          platform: accountConfig2.name,
          error: `\u8D26\u6237\u9650\u6D41: ${accountQuota.message}`
        });
        continue;
      }
      const token = env[accountConfig2.tokenKey];
      if (!token) {
        allResults.failed.push(...Object.keys(accountConfig2.channels));
        allResults.details.push({
          platform: accountConfig2.name,
      ...
      const results = await publishToBuffer(
        Object.values(accountConfig2.channels).map((c) => c.id),
        postText,
        mediaUrl,
        token,
        accountConfig2.channels,
        env,
        accountKey,
        postUrl
      );
...
async function publishToBuffer(channelIds, text, mediaUrl, token, channels, env, accountKey, postUrl) {
  ...
  for (const channelId of channelIds) {
    ...
    const publishTime = new Date(now.getTime() + (accountConfig.scheduleOffset || 0) * 36e5 + (channelIds.indexOf(channelId) + 1) * 72e5);
    const baseInput = {
      channelId,
