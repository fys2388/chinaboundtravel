  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/list-posts") {
      ...
      for (const [key, config] of Object.entries(BUFFER_ACCOUNTS)) {
        ...
        try {
          const orgQuery = 'query { channels(input: { organizationId: "6a17ddf5e051bed5895272f0" }) { id name service } }';
          const orgResp = await fetch(BUFFER_API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
            body: JSON.stringify({ query: orgQuery })
          });
          const orgData = await orgResp.json();
          const postsQuery = "query GetScheduledPosts($input: PostsInput!) { posts(input: $input) { id text dueAt isSent hasFailed channel { id name service } assets { type image { url } } } } }";
          const postsResp = await fetch(BUFFER_API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
            body: JSON.stringify({
              query: postsQuery,
              variables: { input: { organizationId: "6a17ddf5e051bed5895272f0", pageSize: 30, filter: { status: "scheduled" } } }
            })
          });
          const postsData = await postsResp.json();
          results[key] = postsData;
        } catch (e) {
          results[key] = { error: e.message };
        }
      }
      const tokenB = env.BUFFER_WORKER_URL || "";
      try {
        const postsResp = await fetch(BUFFER_API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${tokenB}` },
          body: JSON.stringify({
            query: "query GetScheduledPosts($input: PostsInput!) { posts(input: $input) { id text dueAt isSent hasFailed channel { id name service } assets { type image { url } } } } }",
            variables: { input: { organizationId: "6a20329943b37a7289e25b6d", pageSize: 30, filter: { status: "scheduled" } } }
          })
        });
        const postsData = await postsResp.json();
        results["Buffer-B-Pinterest"] = postsData;
      } catch (e) {
        results["Buffer-B-Pinterest"] = { error: e.message };
      }
