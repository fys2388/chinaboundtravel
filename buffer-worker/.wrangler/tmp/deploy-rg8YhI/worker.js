...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/debug-boards") {
      ...
      const queries = [
        { name: "channels_list", query: 'query { channels(input: { service: "pinterest" }) { id name service metadata { ... on PinterestMetadata { boards { id name serviceId url } } } } }' },
        { name: "all_channels", query: "query { channels(input: {}) { id name service } }" }
      ];
      const results = {};
