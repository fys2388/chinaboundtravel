#!/usr/bin/env python3
"""
⚠️  DEPRECATED (2026-09-20): 本脚本与 seo_intelligent_agent.py 100%职责重叠。
   策略文件由 core agent 内部通过 StrategyConsumer 消费。
   工作流已移除对此脚本的调用。保留仅供手动调试使用。
   参见: scripts/seo_intelligent_agent.py
...
class SEOLearningClosedLoop:
    ...
    def decide_and_update_strategy(self, insights: Dict) -> Dict:
        ...
        try:
            save_rollback(self.current_strategy, str(STRATEGY_FILE), "seo", self.current_strategy.get("strategy_changes", []))
        ...
        if should_block_strategy_update(_dq_records, "seo"):
