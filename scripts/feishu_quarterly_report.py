# -*- coding: utf-8 -*-
"""
feishu_quarterly_report.py - ChinaBound Travel 飞书季度报推送
功能：季度运营数据 + 年度 OKR 进度 + 上季度计划复盘 + 下季度计划
版本：v1.0
"""
import os
import sys
import re

sys.stdout.reconfigure(encoding="utf-8")
sys.stderr.reconfigure(encoding="utf-8")

import json
import requests
import hashlib
import hmac
import base64
from datetime import datetime, timedelta
from pathlib import Path

import reporting_snapshot_reader

try:
    from google.oauth2 import service_account
    from google.auth.transport.requests import Request
    HAS_GOOGLE_AUTH = True
except ImportError:
    HAS_GOOGLE_AUTH = False

SCRIPT_DIR = Path(__file__).parent.resolve()
BLOG_ROOT = SCRIPT_DIR.parent

# OKR 公共工具
sys.path.insert(0, str(SCRIPT_DIR))
import okr_utils
import report_advice

try:
    from dotenv import load_dotenv
    load_dotenv(BLOG_ROOT / ".env")
except ImportError:
    pass

CONTENT_DIR = BLOG_ROOT / "content"
POSTS_DIR = CONTENT_DIR / "posts"
REPORTS_DIR = BLOG_ROOT / "reports"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

FEISHU_WEBHOOK_URL = os.environ.get("FEISHU_WEBHOOK_URL", "")
FEISHU_SECRET = os.environ.get("FEISHU_SECRET", "")
TRAVELPAYOUTS_API_TOKEN = os.environ.get("TRAVELPAYOUTS_API_TOKEN", "")
TRAVELPAYOUTS_MARKER = os.environ.get("TRAVELPAYOUTS_MARKER", "730795")
GA4_PROPERTY_ID = os.environ.get("GA4_PROPERTY_ID", "538482322")
GA4_SERVICE_ACCOUNT_JSON = os.environ.get("GA4_SERVICE_ACCOUNT_JSON", "")
MAILERLITE_API_TOKEN = os.environ.get("MAILERLITE_API_TOKEN", "")
# 2026-09-23 AUDIT-OPS-005：接入 ml_utils.clean_token，语义与
# functions/api/subscribe.js:cleanToken 完全一致（剥离 BOM + 空白 + 剔非可打印 ASCII）
_SCRIPTS_DIR_LOCAL = os.path.dirname(os.path.abspath(__file__))
if _SCRIPTS_DIR_LOCAL not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR_LOCAL)
from ml_utils import clean_token as _ml_clean_token  # noqa: E402
MAILERLITE_API_TOKEN = _ml_clean_token(MAILERLITE_API_TOKEN)
GSC_SERVICE_ACCOUNT_JSON = os.environ.get("GSC_SERVICE_ACCOUNT_JSON", "")
GSC_SITE_URL = os.environ.get("GSC_SITE_URL", "sc-domain:chinaboundtravel.com")


class FeishuQuarterlyReporter:
    def __init__(self):
        self.last_quarter_data = {}
        self.last_quarter_file = REPORTS_DIR / "last_quarter_data.json"
        if self.last_quarter_file.exists():
            try:
                self.last_quarter_data = json.loads(self.last_quarter_file.read_text(encoding="utf-8"))
            except Exception:
                pass

    # ---------- 周期计算 ----------
    def _get_period(self):
        """报告上一完整季度，对比再上一季度"""
        now = datetime.now()
        cur_q = (now.month - 1) // 3 + 1
        # 上季度起止
        if cur_q == 1:
            rep_end = datetime(now.year - 1, 12, 31)
            rep_start = datetime(now.year - 1, 10, 1)
            cmp_end = datetime(now.year - 1, 9, 30)
            cmp_start = datetime(now.year - 1, 7, 1)
            rep_label = f"{now.year - 1}年Q4"
            cmp_label = f"{now.year - 1}年Q3"
        else:
            rep_start_month = (cur_q - 2) * 3 + 1
            rep_start = datetime(now.year, rep_start_month, 1)
            rep_end = datetime(now.year, rep_start_month + 3, 1) - timedelta(days=1)
            cmp_start_month = rep_start_month - 3
            cmp_start = datetime(now.year, cmp_start_month, 1)
            cmp_end = rep_start - timedelta(days=1)
            rep_label = f"{now.year}年Q{cur_q - 1}"
            cmp_label = f"{now.year}年Q{cur_q - 2}"
        return {
            "rep_start": rep_start.strftime("%Y-%m-%d"), "rep_end": rep_end.strftime("%Y-%m-%d"),
            "cmp_start": cmp_start.strftime("%Y-%m-%d"), "cmp_end": cmp_end.strftime("%Y-%m-%d"),
            "rep_label": rep_label, "cmp_label": cmp_label,
        }

    # ---------- 飞书签名与推送 ----------
    def _generate_signature(self, timestamp: str) -> str:
        string_to_sign = f"{timestamp}\n{FEISHU_SECRET}"
        hmac_code = hmac.new(string_to_sign.encode("utf-8"), digestmod=hashlib.sha256).digest()
        return base64.b64encode(hmac_code).decode("utf-8")

    def send_card_message(self, card_content: dict) -> bool:
        if not FEISHU_WEBHOOK_URL:
            print("   ⚠️ FEISHU_WEBHOOK_URL 未配置")
            return False
        try:
            payload = {"msg_type": "interactive", "card": card_content}
            timestamp = str(round(datetime.now().timestamp()))
            if FEISHU_SECRET:
                payload["timestamp"] = timestamp
                payload["sign"] = self._generate_signature(timestamp)
            resp = requests.post(FEISHU_WEBHOOK_URL, json=payload, timeout=15)
            print(f"📤 飞书响应状态码: {resp.status_code}")
            print(f"📤 飞书响应内容: {resp.text[:200]}")
            return resp.status_code == 200 and '"StatusCode":0' in resp.text
        except Exception as e:
            print(f"   ⚠️ 飞书发送失败: {e}")
            return False

    # ---------- GA4 ----------
    def _load_service_account(self, sa_json_str: str):
        if not sa_json_str:
            return None
        try:
            return json.loads(sa_json_str)
        except Exception:
            pass
        try:
            if "\\n" in sa_json_str and "BEGIN PRIVATE KEY" in sa_json_str:
                return json.loads(sa_json_str.replace("\\n", "\n"))
        except Exception:
            pass
        try:
            sa_path = Path(sa_json_str)
            if not sa_path.is_absolute():
                sa_path = BLOG_ROOT / sa_path
            if sa_path.exists() and sa_path.is_file():
                return json.loads(sa_path.read_text(encoding="utf-8"))
        except Exception as e:
            print(f"   ⚠️ 服务账号加载失败: {e}")
        return None

    def _get_ga4_headers(self):
        if not GA4_SERVICE_ACCOUNT_JSON or not HAS_GOOGLE_AUTH:
            print("   ⚠️ GA4: 服务账号未配置")
            return None
        try:
            info = self._load_service_account(GA4_SERVICE_ACCOUNT_JSON)
            if not info:
                return None
            creds = service_account.Credentials.from_service_account_info(
                info, scopes=["https://www.googleapis.com/auth/analytics.readonly"])
            creds.refresh(Request())
            return {"Authorization": f"Bearer {creds.token}", "Content-Type": "application/json"}
        except Exception as e:
            print(f"   ⚠️ GA4 认证失败: {e}")
        return None

    def _ga4_run_report(self, headers: dict, payload: dict):
        url = f"https://analyticsdata.googleapis.com/v1beta/properties/{GA4_PROPERTY_ID}:runReport"
        try:
            resp = requests.post(url, headers=headers, json=payload, timeout=30)
            if resp.status_code == 200:
                return resp.json()
            print(f"   ⚠️ GA4 API 响应 {resp.status_code}: {resp.text[:200]}")
        except Exception as e:
            print(f"   ⚠️ GA4 API 调用失败: {e}")
        return None

    def _fetch_quarterly_ga4(self) -> dict:
        """本季度（报告期）核心指标 + 对比上季度"""
        headers = self._get_ga4_headers()
        if not headers:
            return None
        p = self._get_period()
        print(f"   🔍 GA4 季度数据（{p['rep_label']}: {p['rep_start']} ~ {p['rep_end']}）...")

        def fetch_range(start, end):
            payload = {
                "dateRanges": [{"startDate": start, "endDate": end}],
                "metrics": [
                    {"name": "activeUsers"}, {"name": "sessions"}, {"name": "screenPageViews"},
                    {"name": "engagementRate"}, {"name": "averageSessionDuration"}, {"name": "bounceRate"}],
            }
            res = self._ga4_run_report(headers, payload)
            if res and "rows" in res:
                r = res["rows"][0]
                return {
                    "users": int(r["metricValues"][0].get("value", "0")),
                    "sessions": int(r["metricValues"][1].get("value", "0")),
                    "pageviews": int(r["metricValues"][2].get("value", "0")),
                    "engagement": round(float(r["metricValues"][3].get("value", "0")) * 100, 1),
                    "avg_duration": int(float(r["metricValues"][4].get("value", "0"))),
                    "bounce": round(float(r["metricValues"][5].get("value", "0")) * 100, 1),
                }
            return {"users": 0, "sessions": 0, "pageviews": 0, "engagement": 0.0, "avg_duration": 0, "bounce": 0.0}

        cur = fetch_range(p["rep_start"], p["rep_end"])
        prev = fetch_range(p["cmp_start"], p["cmp_end"])

        def change(c, v):
            if v == 0:
                return "-"
            chg = round((c - v) / v * 100, 1)
            icon = "📈" if chg > 0 else "📉" if chg < 0 else "➡️"
            return f"{icon} {abs(chg)}%"

        # 渠道 Top5
        channels = []
        ch_res = self._ga4_run_report(headers, {
            "dateRanges": [{"startDate": p["rep_start"], "endDate": p["rep_end"]}],
            "metrics": [{"name": "activeUsers"}],
            "dimensions": [{"name": "sessionDefaultChannelGroup"}],
            "orderBys": [{"metric": {"metricName": "activeUsers"}, "desc": True}], "limit": 5})
        if ch_res and "rows" in ch_res:
            for row in ch_res["rows"]:
                channels.append({"channel": row["dimensionValues"][0].get("value", "N/A"),
                                 "users": int(row["metricValues"][0].get("value", "0"))})

        # Top 页面
        pages = []
        pg_res = self._ga4_run_report(headers, {
            "dateRanges": [{"startDate": p["rep_start"], "endDate": p["rep_end"]}],
            "metrics": [{"name": "screenPageViews"}],
            "dimensions": [{"name": "pagePath"}],
            "orderBys": [{"metric": {"metricName": "screenPageViews"}, "desc": True}], "limit": 5})
        if pg_res and "rows" in pg_res:
            for row in pg_res["rows"]:
                pages.append({"path": row["dimensionValues"][0].get("value", ""),
                              "views": int(row["metricValues"][0].get("value", "0"))})

        return {
            "quarter_label": p["rep_label"], "prev_quarter_label": p["cmp_label"],
            "quarter_start": p["rep_start"], "quarter_end": p["rep_end"],
            "quarter_users": cur["users"], "quarter_sessions": cur["sessions"],
            "quarter_pageviews": cur["pageviews"], "quarter_bounce": cur["bounce"],
            "quarter_engagement": cur["engagement"], "quarter_avg_duration": cur["avg_duration"],
            "prev_users": prev["users"], "prev_pageviews": prev["pageviews"], "prev_bounce": prev["bounce"],
            "users_change": change(cur["users"], prev["users"]),
            "pageviews_change": change(cur["pageviews"], prev["pageviews"]),
            "channels": channels, "top_pages": pages,
        }

    # ---------- GSC ----------
    def _fetch_gsc_data(self) -> dict:
        if not GSC_SERVICE_ACCOUNT_JSON or not HAS_GOOGLE_AUTH:
            return {"status": "unauthorized"}
        try:
            info = self._load_service_account(GSC_SERVICE_ACCOUNT_JSON)
            if not info:
                return {"status": "unauthorized"}
            creds = service_account.Credentials.from_service_account_info(
                info, scopes=["https://www.googleapis.com/auth/webmasters.readonly"])
            creds.refresh(Request())
            from googleapiclient.discovery import build
            service = build("searchconsole", "v1", credentials=creds)
            candidates = [s.strip() for s in str(GSC_SITE_URL or "").split(",") if s.strip()]
            if "sc-domain:chinaboundtravel.com" not in candidates:
                candidates.append("sc-domain:chinaboundtravel.com")
            site_url = None
            for cand in candidates:
                try:
                    service.sites().get(siteUrl=cand).execute()
                    site_url = cand
                    break
                except Exception:
                    continue
            if not site_url:
                return {"status": "unauthorized"}
            p = self._get_period()
            resp = service.searchanalytics().query(
                siteUrl=site_url,
                body={"startDate": p["rep_start"], "endDate": p["rep_end"],
                      "rowLimit": 1000, "dataState": "final"}).execute()
            impressions = 0
            clicks = 0
            if "rows" in resp:
                impressions = int(resp["rows"][0].get("impressions", 0))
                clicks = int(resp["rows"][0].get("clicks", 0))
            return {"status": "authorized", "gsc_impressions": impressions, "gsc_clicks": clicks}
        except Exception as e:
            print(f"   ⚠️ GSC API 获取失败: {e}")
            return {"status": "error"}

    # ---------- Travelpayouts ----------
    def _fetch_quarterly_travelpayouts(self) -> dict:
        if not TRAVELPAYOUTS_API_TOKEN:
            return {"tp_available": False, "tp_error": "TRAVELPAYOUTS_API_TOKEN 未配置"}
        try:
            p = self._get_period()
            resp = requests.post(
                "https://api.travelpayouts.com/statistics/v1/execute_query",
                headers={"X-Access-Token": TRAVELPAYOUTS_API_TOKEN, "Content-Type": "application/json"},
                json={"fields": ["redirects_count", "inits_count", "searches_count",
                                 "paid_actions_count", "paid_profit_usd_sum"],
                      "filters": [
                          {"field": "date", "op": "ge", "value": p["rep_start"]},
                          {"field": "date", "op": "le", "value": p["rep_end"]}],
                      "offset": 0, "limit": 1},
                timeout=30)
            if resp.status_code != 200:
                return {"tp_available": False, "tp_error": f"HTTP {resp.status_code}"}
            rows = resp.json().get("results", []) or resp.json().get("data", [])
            row = rows[0] if rows else {}
            return {"tp_available": True,
                    "quarter_revenue": float(row.get("paid_profit_usd_sum", 0) or 0),
                    "tp_clicks": int(row.get("redirects_count", 0) or 0),
                    "tp_bookings": int(row.get("paid_actions_count", 0) or 0)}
        except Exception as e:
            print(f"   ⚠️ Travelpayouts API 获取失败: {e}")
            return {"tp_available": False, "tp_error": str(e)[:100]}

    # ---------- MailerLite ----------
    def _fetch_quarterly_mailerlite(self) -> dict:
        """2026-09-23 AUDIT-OPS-005：MAILERLITE_API_TOKEN 已在模块顶层用
        ml_utils.clean_token() 剥离 BOM，这里直接复用。同时把 x-total-count
        响应头改为 cursor 分页遍历 data.data 计数（MailerLite API v3 已移除
        该 header）。
        """
        if not MAILERLITE_API_TOKEN:
            return {"ml_available": False, "ml_error": "MAILERLITE_API_TOKEN 未配置"}
        try:
            headers = {"Authorization": f"Bearer {MAILERLITE_API_TOKEN}", "Content-Type": "application/json"}
            total = 0
            cursor = None
            for _ in range(20):  # 安全上限 ~20k subscribers
                params = {"limit": 1000}
                if cursor:
                    params["cursor"] = cursor
                resp = requests.get("https://connect.mailerlite.com/api/subscribers",
                                    headers=headers, params=params, timeout=15)
                if resp.status_code != 200:
                    return {"ml_available": False, "ml_error": f"MailerLite API 认证失败（HTTP {resp.status_code}）"}
                data = resp.json()
                batch = data.get("data", [])
                for s in batch:
                    if s.get("status") == "active":
                        total += 1
                cursor = (data.get("meta") or {}).get("next_cursor")
                if not cursor or len(batch) < 1000:
                    break
            return {"ml_available": True, "ml_total_subscribers": total}
        except Exception as e:
            return {"ml_available": False, "ml_error": str(e)[:100]}

    # ---------- 内容巡检 ----------
    def _scan_content_quality(self) -> dict:
        if not POSTS_DIR.exists():
            return {"total_posts": 0, "quarter_new_posts": 0, "missing_alt": 0}
        p = self._get_period()
        posts = list(POSTS_DIR.glob("*.md"))
        new_count = 0
        missing_alt = 0
        for post in posts:
            try:
                content = post.read_text(encoding="utf-8")
                fm = re.match(r"^---\s*\n(.*?)\n---", content, re.DOTALL)
                post_date = None
                if fm:
                    dm = re.search(r"^date:\s*[\"']?([\d-]+)", fm.group(1), re.MULTILINE)
                    if dm:
                        post_date = dm.group(1)
                if post_date and p["rep_start"] <= post_date <= p["rep_end"]:
                    new_count += 1
                for img in re.finditer(r"!\[([^\]]*)\]\([^)]+\)", content):
                    if not img.group(1).strip():
                        missing_alt += 1
            except Exception:
                continue
        return {"total_posts": len(posts), "quarter_new_posts": new_count, "missing_alt": missing_alt}

    # ---------- 汇总 ----------
    def collect_data(self) -> dict:
        print("📊 收集季度报数据...")
        data = {}
        # 2.0 统一口径（P1-REPORT-03R）：SNAPSHOT 优先，命中则 GA4/GSC 采用统一快照，不再直连
        _snap_ga4 = reporting_snapshot_reader.snapshot_traffic("quarter")
        if _snap_ga4 is not None:
            print("   📦 2.0 统一快照命中：GA4/GSC 采用 SNAPSHOT 口径（as_of=%s）" % _snap_ga4.get("snapshot_as_of"))
            data.update(_snap_ga4)
            _snap_gsc = reporting_snapshot_reader.snapshot_gsc()
            if _snap_gsc is not None:
                data.update(_snap_gsc)
            else:
                data.update(self._fetch_gsc_data())
        else:
            ga4 = self._fetch_quarterly_ga4()
            if ga4:
                data.update(ga4)
            gsc = self._fetch_gsc_data()
            data.update(gsc)
        tp = self._fetch_quarterly_travelpayouts()
        if tp:
            data.update(tp)
        ml = self._fetch_quarterly_mailerlite()
        if ml:
            data.update(ml)
        content = self._scan_content_quality()
        data["content_data"] = content
        data.update({"total_posts": content["total_posts"],
                     "quarter_new_posts": content["quarter_new_posts"],
                     "missing_alt": content["missing_alt"]})

        # OKR 板块（报告季度=上季度达成）+ 上季度计划复盘
        p = self._get_period()
        rep_end_dt = datetime.strptime(p["rep_end"], "%Y-%m-%d")
        data["okr_section"] = okr_utils.build_okr_section(data, "quarterly", report_date=rep_end_dt)
        prev_key = okr_utils.period_key("quarterly", rep_end_dt - timedelta(days=1))
        prev_snap = okr_utils.load_snapshot("quarterly", prev_key)
        data["okr_review"] = okr_utils.review_previous_plan(prev_snap, data)

        # 本季度计划（按当前季度 OKR 差距生成）
        plan = []
        okr = okr_utils.load_okr()
        if okr:
            cur_q = (datetime.now().month - 1) // 3 + 1
            quarter_krs = okr_utils.pick_quarter_krs(okr, cur_q)
            for kr in quarter_krs:
                cur = okr_utils.extract_kr(data, kr.get("source", ""))
                target = float(kr.get("target", 0) or 0)
                if target > 0 and cur / 3.0 < target * 0.5:
                    plan.append({"task": f"提升「{kr.get('name')}」至月度目标的50%以上（上季月均{cur/3.0:g}，目标{target:g}）",
                                 "priority": "high", "period": "本季度", "kr_id": kr.get("id"), "target": target})
        if not plan:
            plan = [{"task": "持续内容产出与SEO优化，巩固季度增长", "priority": "medium", "period": "本季度"}]
        data["next_quarter_plan"] = plan

        # 自动运营建议（基于真实数据精准生成）
        data["advice_section"] = report_advice.advice_section(data, "quarterly")
        return data

    # ---------- 卡片 ----------
    def build_quarterly_card(self, data: dict) -> dict:
        p = self._get_period()
        rep_label = data.get("quarter_label", p["rep_label"])
        users = data.get("quarter_users", 0)
        sessions = data.get("quarter_sessions", 0)
        pageviews = data.get("quarter_pageviews", 0)
        bounce = data.get("quarter_bounce", 0)
        dur = data.get("quarter_avg_duration", 0)
        dur_str = f"{dur // 60}分{dur % 60}秒" if dur >= 60 else f"{dur}秒"
        tp_ok = data.get("tp_available", False)
        tp_rev = data.get("quarter_revenue", 0)
        gsc_ok = data.get("status") == "authorized"
        gsc_imp = data.get("gsc_impressions", 0)
        ml_ok = data.get("ml_available", False)
        ml_total = data.get("ml_total_subscribers", 0)
        content = data.get("content_data", {})
        total_posts = content.get("total_posts", 0)
        new_posts = content.get("quarter_new_posts", 0)

        channel_rows = "\n".join([f"| {c['channel']} | {c['users']} 人 |"
                                  for c in data.get("channels", [])[:5]]) or "| - | - |"
        page_rows = "\n".join([f"| {i}. {pth['path']} | {pth['views']} 次 |"
                               for i, pth in enumerate(data.get("top_pages", [])[:5], 1)]) or "| - | - |"
        okr_review_rows = "\n".join([f"| {r['task']} | {r['icon']} | {r['status']} | {r['period']} |"
                                     for r in data.get("okr_review", [])]) or "| - | - | - | - |"
        plan_rows = "\n".join([f"| {it['task']} | {'🔴' if it['priority']=='high' else '🟡'} | {it['period']} |"
                               for it in data.get("next_quarter_plan", [])]) or "| - | - | - |"

        main = f"""**📊 ChinaBound Travel 季度运营报告 | {rep_label}**

| 指标 | 本季 | 上季 | 环比 |
| :--- | :--- | :--- | :--- |
| 访客数 | {users} 人 | {data.get('prev_users', 0)} | {data.get('users_change', '-')} |
| 页面浏览 | {pageviews} 次 | {data.get('prev_pageviews', 0)} | {data.get('pageviews_change', '-')} |
| 跳出率 | {bounce}% | {data.get('prev_bounce', 0)}% | - |
| 平均时长 | {dur_str} | - | - |
| 会话数 | {sessions} 次 | - | - |

**🔗 渠道 Top5**
{channel_rows}

**🔥 热门页面 Top5**
{page_rows}"""

        seo = f"""---
## 🔍 SEO 与增长
- **GSC 状态**：{'已授权' if gsc_ok else '未授权'}｜本季曝光 **{gsc_imp} 次**
- **内容**：总文章 {total_posts} 篇｜本季新增 {new_posts} 篇
- **联盟**：{'已接入' if tp_ok else '未接入'}｜本季佣金 **${tp_rev:.2f}**
- **订阅**：{'已接入' if ml_ok else '未接入'}｜总订阅 {ml_total} 人"""

        okr_section = data.get("okr_section", "")

        review = f"""---
## ✅ 上季度计划复盘（OKR 对齐）
| 计划项 | 优先级 | 状态 | 周期 |
| :--- | :--- | :--- | :--- |
{okr_review_rows}"""

        plan = f"""---
## 📋 下季度执行计划
| 任务项 | 优先级 | 周期 |
| :--- | :--- | :--- |
{plan_rows}"""

        card = {
            "config": {"wide_screen_mode": True},
            "header": {"template": "blue", "title": {"tag": "plain_text",
                       "content": f"📊 ChinaBound Travel 季度运营报告 | {rep_label}"}},
            "elements": [
                {"tag": "div", "text": {"tag": "lark_md", "content": main}},
                ({"tag": "div", "text": {"tag": "lark_md", "content": okr_section}} if okr_section else None),
                ({"tag": "div", "text": {"tag": "lark_md", "content": data.get("advice_section", "")}} if data.get("advice_section") else None),
                {"tag": "div", "text": {"tag": "lark_md", "content": seo}},
                {"tag": "div", "text": {"tag": "lark_md", "content": review}},
                {"tag": "div", "text": {"tag": "lark_md", "content": plan}},
            ],
        }
        card["elements"] = [el for el in card["elements"] if el]
        return card

    # ---------- 执行 ----------
    def run(self) -> bool:
        print("=" * 60)
        print("🌍 ChinaBound Travel 季度运营报告")
        print("=" * 60)
        data = self.collect_data()
        p = self._get_period()
        rep_end_dt = datetime.strptime(p["rep_end"], "%Y-%m-%d")
        okr_utils.save_snapshot("quarterly", okr_utils.period_key("quarterly", rep_end_dt),
                                data.get("next_quarter_plan", []), data)
        print("📝 构建飞书季度报卡片...")
        card = self.build_quarterly_card(data)
        print("📤 发送飞书消息...")
        success = self.send_card_message(card)
        report_path = REPORTS_DIR / f"quarterly_report_{datetime.now().strftime('%Y%m%d')}.json"
        with open(report_path, "w", encoding="utf-8") as fh:
            json.dump(data, fh, indent=2, ensure_ascii=False)
        print(f"📁 季度报已保存: {report_path}")
        print("=" * 60)
        print(f"{'✅ 季度报推送完成' if success else '❌ 季度报推送失败'}")
        print("=" * 60)
        return success


def main():
    reporter = FeishuQuarterlyReporter()
    return 0 if reporter.run() else 1


if __name__ == "__main__":
    sys.exit(main())
