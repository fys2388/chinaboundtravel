#!/usr/bin/env python3
"""
⚠️  DEPRECATED (2026-09-20): 本脚本与 revenue_analytics_engine.py 100%职责重叠。
   策略文件由 core agent 内部通过 StrategyConsumer 消费。
   工作流已移除对此脚本的调用。保留仅供手动调试使用。
   参见: scripts/revenue_analytics_engine.py
...
class RevenueLearningClosedLoop:
    ...
    def decide_and_update_strategy(self, insights: Dict) -> Dict:
        ...
        if should_block_strategy_update(_dq_records, "revenue"):
